...
//        given().cookie("XSRF-TOKEN",TOKEN, "JSESSIONID", JSESSIONID ).`when`().get("/realization/24").then().assertThat().statusCode(200)
//        given().header("XSRF-TOKEN",TOKEN).header("JSESSIONID", JSESSIONID).`when`().get("/realization/24").then().assertThat().statusCode(200)
}